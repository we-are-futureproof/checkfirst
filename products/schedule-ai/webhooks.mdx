...
| :-------------------------- | :-------------------------------------------------- | :--------------------------------------------------------------- |
| `REPORT_CREATED`            | A user submits a report from the mobile or web app. | Pull the full report JSON / PDF and archive it in your DMS.      |
| `ISSUE_CREATED`             | An issue is flagged during an inspection.           | Create a corrective-action ticket and assign it to a supervisor. |
| `COUNTER_SIGNATURE_CREATED` | A report is counter-signed in the dashboard.        | Mark the job as “Closed” in your ERP and notify the client.      |
...
| `type`      | Event name (see table above).             |
